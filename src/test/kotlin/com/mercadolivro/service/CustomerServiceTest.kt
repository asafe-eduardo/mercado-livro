...
class CustomerServiceTest {
    ...
    fun `should create customer and encrypt password`() {
        ...
        val faCustomerEncrypted = fakeCustomer.copy(password = fakePassword)

        every { customerRepository.save(faCustomerEncrypted) } returns fakeCustomer
        every { bCrypt.encode(initialPassWord) } returns fakePassword

        customerService.createCustomer(fakeCustomer)

        verify(exactly = 1) { customerRepository.save(faCustomerEncrypted) }
    ...
    @Test
    fun `should throw error when customer not found when find by id`() {
        val id = Random().nextInt()
    ...
    fun `should delete customer`() {
        val id = Random().nextInt()
        val fakeCustomer = buildCustomer(id = id)
        val expectedCustomer = fakeCustomer.copy(status = CustomerStatus.INTAIVO)
